...
export class Player {
  constructor(scene, position = { x: 0, y: 0, z: 4 }) {
    ...
    this.gravity =
      -(2 * this.JUMP_HEIGHT) / Math.pow(this.JUMP_DURATION / 2, 2);
  ...
  resetScale() {
    const animate = () => {
      ...
      this.playerGroup.scale.y +=
        (targetScale - this.playerGroup.scale.y) * springSpeed;
      this.playerGroup.scale.x +=
        (targetScale - this.playerGroup.scale.x) * springSpeed;
